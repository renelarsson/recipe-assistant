"""
Grafana initialization script for Recipe Assistant.
- Sets up API key, PostgreSQL datasource, and dashboard in Grafana.
- Should be run after all services (Grafana, Postgres, app) are up.
- Reads configuration from .env and dashboard.json.
"""
...
# Grafana and Postgres config from environment
...
def create_api_key():
    """
    Create or update an API key for Grafana programmatic access.
...
def create_or_update_datasource(api_key):
    ...
    # Try to get the existing datasource
...
def create_dashboard(api_key, datasource_uid):
    """
    Create or update the Grafana dashboard for Recipe Assistant.
    Updates the dashboard title and datasource UID.
    """
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
    ...
    # Update title to "Recipe Assistant"
    dashboard_json["title"] = "Recipe Assistant"
...
def main():
    """
    Main entry point: creates API key, datasource, and dashboard in Grafana.
    """
